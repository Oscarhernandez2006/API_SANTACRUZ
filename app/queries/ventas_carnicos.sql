...
items_especie AS (
    SELECT DISTINCT 
        ic.f125_rowid_item,
        cat.f106_id          AS id_especie,
        cat.f106_descripcion AS especie
...
items_proceso AS (
    SELECT DISTINCT 
        ic.f125_rowid_item,
        cat.f106_id          AS id_proceso,
        cat.f106_descripcion AS proceso
    FROM dbo.t125_mc_items_criterios ic
    INNER JOIN dbo.t106_mc_criterios_item_mayores cat 
            ON cat.f106_id_plan = ic.f125_id_plan
           AND cat.f106_id      = ic.f125_id_criterio_mayor
    WHERE ic.f125_id_plan = '003'
      AND cat.f106_id_cia = :id_cia
),
items_grupo AS (
    SELECT DISTINCT 
        ic.f125_rowid_item,
        cat.f106_id          AS id_grupo,
        cat.f106_descripcion AS grupo
    FROM dbo.t125_mc_items_criterios ic
    INNER JOIN dbo.t106_mc_criterios_item_mayores cat 
            ON cat.f106_id_plan = ic.f125_id_plan
           AND cat.f106_id      = ic.f125_id_criterio_mayor
    WHERE ic.f125_id_plan = '001'
      AND cat.f106_id_cia = :id_cia
),
ventas AS (
    SELECT 
        ...
        m.f470_rowid_contacto_docto                                             AS rowid_contacto_docto,
...
SELECT 
    ...
    v.rowid_vendedor                                           AS rowid_vendedor,
...
INNER JOIN items_grupo                   ig   ON ig.f125_rowid_item    = item.f120_rowid   -- INNER: excluye SIN GRUPO
...
LEFT  JOIN dbo.t200_mm_terceros          tv   ON tv.f200_rowid = v.rowid_vendedor 
                                             AND tv.f200_id_cia = v.f470_id_cia
LEFT  JOIN dbo.t015_mm_contactos         cd   ON cd.f015_rowid = v.rowid_contacto_docto 
                                             AND cd.f015_id_cia = v.f470_id_cia
LEFT  JOIN dbo.t201_mm_clientes          cli  ON cli.f201_rowid_contacto = cd.f015_rowid 
                                             AND cli.f201_id_cia = v.f470_id_cia
LEFT  JOIN dbo.t200_mm_terceros          tc   ON tc.f200_rowid = cli.f201_rowid_tercero 
...
GROUP BY 
    ...
    v.rowid_vendedor, tv.f200_id, tv.f200_razon_social,
    tc.f200_id, tc.f200_nit, tc.f200_razon_social
ORDER BY 
    v.f470_id_cia, co.f285_id, v.fecha,
    especie, proceso, item.f120_descripcion,
    tv.f200_razon_social, tc.f200_razon_social;
